...
func Test_Withdrawal_Enforcement(t *testing.T) {
	...
	transactor.Account.L1Opts, err = bind.NewKeyedTransactorWithChainID(transactor.Account.Key, ts.Cfg.L1ChainIDBig())
	assert.NoError(t, err)
	transactor.Account.L2Opts, err = bind.NewKeyedTransactorWithChainID(transactor.Account.Key, ts.Cfg.L2ChainIDBig())
	assert.NoError(t, err)
	...
	fakeAddr, tx, _, err := bindings.DeployL2ToL1MessagePasser(transactor.Account.L2Opts, l2Seq)
	...
	// Setup Pessimism to listen for the withdrawal
	err = ts.App.BootStrap([]models.InvRequestParams{{
		Network:      "layer1",
		PType:        "live",
	...
	assert.Nil(t, err, "sending prove withdrawal tx")
	...
	// Ensure Pessemism has detected what it considers a "faulty" withdrawal
