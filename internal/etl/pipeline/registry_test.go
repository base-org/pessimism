...
func getTestPipeLine(ctx context.Context) Pipeline {
	...
	blackHolePipe, err := p_registry.NewBlackHoleTxPipe(ctx, component.WithID(cID1))
	if err != nil {
		panic(err)
	}

	contractCreatePipe, err := p_registry.NewCreateContractTxPipe(ctx, component.WithID(cID2))
	if err != nil {
		panic(err)
	}
