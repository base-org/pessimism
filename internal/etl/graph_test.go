...
var (
	testCUUID1 = core.MakeProcessID(69, 69, 69, 69)
	testCUUID2 = core.MakeProcessID(42, 42, 42, 42)
)
	...
	var tests = []struct {
	...
	}{
		{
			...
			testLogic: func(t *testing.T, g *etl.Graph) {
				...
				actualComponent, err := g.GetProcess(cUUID)
		...
		},
		{
			...
			constructionLogic: func() *etl.Graph {
				...
				if err = g.Add(testCUUID1, comp1); err != nil {
					panic(err)
				}
				...
				if err = g.Add(testCUUID2, comp2); err != nil {
					panic(err)
				}

				if err = g.Subscribe(testCUUID1, testCUUID2); err != nil {
					panic(err)
				}
			...
			testLogic: func(t *testing.T, g *etl.Graph) {
				err := g.Subscribe(testCUUID2, testCUUID1)
				assert.Error(t, err)
		...
		},
		{
			...
			testLogic: func(t *testing.T, g *etl.Graph) {
				err := g.Subscribe(testCUUID1, testCUUID2)
				assert.Error(t, err)
		...
		},
		{
			...
			testLogic: func(t *testing.T, g *etl.Graph) {
				comp1, _ := g.GetProcess(testCUUID1)

				err := g.Subscribe(testCUUID1, testCUUID2)
				assert.NoError(t, err)

				err = comp1.AddSubscriber(testCUUID2, core.NewTransitChannel())
				assert.Error(t, err)

				assert.True(t, g.Exists(testCUUID1))
				assert.True(t, g.Exists(testCUUID2))
